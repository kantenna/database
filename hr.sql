...
-- 3. 급여가 5000 ~ 12000 범위가 아닌 사람들의 LAST_NAME, 급여 조회
SELECT
	e.LAST_NAME ,
	e.SALARY
FROM
...
-- 4. 20번 이나 50번 부서에 근무하는 사원들의 LAST_NAME, 부서번호를 오름차순으로 조회
SELECT
	e.LAST_NAME,
	e.DEPARTMENT_ID
FROM
...
-- 6. 연봉이 2500, 3500, 7000이 아니며 직무가 SA_REP, ST_CLERK인 사원
SELECT
	*
FROM
	EMPLOYEES e
...
-- 8. 20번, 50번 부서에 근무하는 사원들 중에서 연봉이 5000 ~ 12000인 사원들의 LAST_NAME, 급여를 조회 후 급여의 오름차순 정리
SELECT
	e.LAST_NAME ,
	e.SALARY
FROM
...
-- 12. LAST_NAME의 네번째 글자가 a인 사원들의 LAST_NAME 조회
...
-- 15. 매니저 아이다가 없는 사원들의 LAST_NAME을 조회(LAST_NAME, JOB_ID 조회)
SELECT
	e.LAST_NAME ,
	e.JOB_ID
FROM
...
-- 16. 직무가 ST_CLERK인 사원이 근무하고있지 않은 부서 id 조회
-- 단, 부서번호가 NULL인 것은 제외
SELECT
...
-- 17. COMIMISSION_PCT가 NULL 이 아닌 사원들 중에서 
...
-- 함수
-- 18. FIRSTT_NAME이 Curtis인 사람의 FIRST_NAME, LAST_NAME, email, phone_number, job_id 조회
...
-- 단, job_id 가 IT_PROG 인 사원의 경우 프로그래머로 변경하여 출력
SELECT
	e.EMPLOYEE_ID ,
	e.FIRST_NAME ,
	e.HIRE_DATE ,
...
-- 단, 사원명은 FIRST_NAME, LAST_NAME을 연결하여 출력
SELECT
	e.EMPLOYEE_ID ,
...
-- 26. 위치 ID가 1700인 사람들의 사번, LAST_NAME 부서번호, 연봉 조회
SELECT
	e.EMPLOYEE_ID ,
	e.LAST_NAME ,
	d.DEPARTMENT_ID ,
...
-- 27. 부서명, 위치ID, 각 부서별 총 사원수, 부서별 평균 연봉 조회, 평균 연봉은 소수점 2자리 까지만
SELECT
	d.DEPARTMENT_NAME ,
	d.LOCATION_ID ,
	COUNT(e.FIRST_NAME) AS 총사원수,
...
-- 29. 각 사원별 소속부서에서 자신보다 늦게 고용되었으나 보다 많은 연봉을 받는 사원들이 존재하는 모든 사원들의 이름 조회
-- 이름은 FIRST_NAME, LAST_NAME
SELECT
