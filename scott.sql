...
-- 제약조건 (데이터 무결성 유지)
-- 1. 빈 값을 혀용하지 않는 NOT NULL
-- 2. 중복값을 혀용하지 않는 UNIQUE
-- 3. 유일하게 하나만 존재하는 PRIMARY KEY
-- 4. 다른 테이블과 관계를 맺는 FOFREIGN KEY
...
INSERT INTO TABLE_NOT_NULL VALUES('test01', 'test01','010-1234-5678');
INSERT INTO TABLE_NOT_NULL VALUES('test01', 'test01',NULL);

-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("SCOTT"."TABLE_NOT_NULL"."LOGIN_PWD") 안에 삽입할 수 없습니다
INSERT INTO TABLE_NOT_NULL VALUES('test01', NULL ,'010-1234-5678');
-- SQL Error [1407] [72000]: ORA-01407: NULL로 ("SCOTT"."TABLE_NOT_NULL"."LOGIN_ID")을 업데이트할 수 없습니다
UPDATE TABLE_NOT_NULL SET LOGIN_ID = NULL WHERE TEL = '010-1234-5678';

DELETE FROM TABLE_NOT_NULL WHERE LOGIN_ID = 'test01';

DROP TABLE TABLE_NOT_NULL;
...
-- TEL 제약조건 추가
ALTER TABLE TABLE_NOT_NULL MODIFY(TEL NOT NULL);
-- TEL 제약조건 추가(이름 지정)
ALTER TABLE TABLE_NOT_NULL MODIFY(TEL CONSTRAINT TBLNN_TEL_NN NOT NULL);
-- 제약조건 이름 변경
ALTER TABLE TABLE_NOT_NULL RENAME CONSTRAINT SYS_C008356 TO TBLNN_TEL_NN;
-- 제약조건 삭제
ALTER TABLE TABLE_NOT_NULL DROP CONSTRAINT TBLNN_TEL_NN;
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(SCOTT.SYS_C008358)에 위배됩니다
INSERT INTO TABLE_UNIQUE VALUES('test01', 'test02','010-1234-1234');
-- UNIQUE는 NULL 값은 중복의 의미를 부여하지 않음
INSERT INTO TABLE_UNIQUE VALUES(NULL, 'test02','010-1234-1234');
INSERT INTO TABLE_UNIQUE VALUES('test02', 'test02','010-1234-1234');
...
-- UNIQUE 제약조건 이름 지정
CREATE TABLE TABLE_UNIQUE(
	LOGIN_ID VARCHAR2(20) CONSTRAINT TBLU_LGNID_U UNIQUE,
	LOGIN_PWD VARCHAR2(20) NOT NULL,
	TEL VARCHAR2(20)
);
...
-- SQL Error [2291] [23000]: ORA-02291: 무결성 제약조건(SCOTT.SYS_C008365)이 위배되었습니다- 부모 키가 없습니다
INSERT  INTO EMP_FK(empno, ename, job, mgr, hiredate, sal, comm, deptno)
VALUES(7201,'홍길동1','PRESIDENT',null,'2000-01-01',1000,100,50);

-- 입력 시 부모 테이블 데티업먼저 입력 후 자식 테이블 데이터 입력
INSERT INTO DEPT_FK VALUES (10,'DB','SEOUL');
INSERT  INTO EMP_FK(empno, ename, job, mgr, hiredate, sal, comm, deptno)
VALUES(7201,'홍길동1','PRESIDENT',null,'2000-01-01',1000,100,10);
...
-- SQL Error [2292] [23000]: ORA-02292: 무결성 제약조건(SCOTT.SYS_C008365)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- 제약조건명 + 부모데이터 삭제시 자식 데이터 처리 방법 지정
-- 1) ON DELETE CASCADE : 부모 데이터 삭제 시 참조하는 데이터도 함께 삭제
-- 2) ON DELETE SET NULL : 부모 데이터 삭제 시 차조하는 데이터에 NULL값 설정 
CREATE TABLE DEPT_FK(
...
-- SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(SCOTT.TBLCK_LOGINPW_CK)이 위배되었습니다
...
INSERT INTO TABLE_DEFAULT VALUES('test02', NULL ,'010-1234-5678');
